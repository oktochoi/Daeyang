...
export default function MediaAwardsDetailPage() {
  const params = useParams();
  const id = params?.id as string;
  const { t, i18n } = useTranslation();
  
  const article = mediaItems.awards.find(item => item.id === Number(id));
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-24 text-center">
          ...
          <Link href="/media/awards" className="text-teal-600 hover:text-teal-700">
            기사 아카이빙 목록으로 돌아가기
          </Link>
        </div>
        <Footer />
      ...
      <section className="pt-32 pb-16 bg-white border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-6">
          ...
          >
            ...
            {i18n.language === 'ko' ? '기사 아카이빙 목록으로' : 'Back to Article Archive'}
          </Link>
          ...
          <article className="bg-white rounded-xl p-8 sm:p-12 shadow-sm border border-gray-200">
            <div className="prose prose-lg max-w-none">
              ...
              <div className="text-gray-600 leading-relaxed whitespace-pre-line">
                {i18n.language === 'ko' 
                  ? `이 기사는 관리자가 추가한 기사 아카이빙입니다. 실제 기사 내용은 관리자 페이지에서 입력할 수 있습니다.

기사 본문 내용이 여기에 표시됩니다. 관리자는 이 영역에 기사의 전체 내용을 입력할 수 있습니다.` 
                  : `This is an archived article added by an administrator. The actual article content can be entered in the admin page.
